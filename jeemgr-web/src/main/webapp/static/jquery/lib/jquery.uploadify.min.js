...
SWFUpload.prototype.initSettings = function() {
	...
	if (!this.settings.preserve_relative_urls) {
		 ...
		 */
		// canceled by by gaohui just for session sended with url to server
		// 20140718
		this.settings.button_image_url = this.settings.button_image_url ? SWFUpload
				.completeURL(this.settings.button_image_url)
				: this.settings.button_image_url;// added by gaohui just for
		// session sended with url
		// to server 20140718
	}
...
SWFUpload.Console.writeLine = function(g) {
	...
	try {
		...
		showAlert("Exception: " + h.name + " Message: " + h.message);
	}
};
(function(c) {
	var b = {
	...
	var a = {
		onDialogOpen : function() {
			var d = this.settings;
			this.queueData.errorMsg = "文件格式有问题，无法上传:";
		...
		onDialogClose : function(d, f, g) {
			...
			if (c.inArray("onDialogClose", e.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					showAlert(this.queueData.errorMsg);
				}
			}
			if (e.onDialogClose) {
