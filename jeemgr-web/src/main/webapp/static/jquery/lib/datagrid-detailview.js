var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	renderRow: function(target, fields, frozen, rowIndex, rowData){
		...
		// add by huangq 对于立体的数据处理
		if(fields.length>0 && fields[0].indexOf('.')>0 && rowData[fields[0]]==undefined ){
			var elements = new Array();
			var parentName;
			var obj = rowData;
			//add by huangq for datagird
			function _parseBigObjNames(obj,parentName,elements){
				for(var name in obj){ 
					var temp = name;	
					if(parentName) {
						temp = parentName+'.'+temp; 
					}
					if(typeof(obj[name])=="object")
						_parseBigObjNames(obj[name],temp,elements );
					else{
						//document.write( temp +" = "+obj[name] +'<br/>');
			...
			_parseBigObjNames(obj,parentName,elements);
			...
			for(var i=0; i<fields.length; i++){
				...
				if (col){	
					for(var ii=0; ii<elements.length; ii++){
						if( elements[ii].name != field ) continue;
						
						var value = elements[ii].value;	// the field value
	...
	fixDetailRowHeight: function(jq, index){
		return jq.each(function(){
			...
			if (tr2.is(':visible')){
				...
				//hard code by huangq
				if(tr2.height()==119){
					rowHeights = 25* $('#ddv-'+index).datagrid('getRows').length;
					height= parseInt(height,10)+parseInt(rowHeights,10);
				}
